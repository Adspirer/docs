...
### Or have it generated from the account itself

The [**Prompt Engineering Playbook**](/agent-skills/prompt-engineering-playbook) is the packaged
version of everything on this page, built from one ad account's real campaigns, spend, and history.
It's a **one-time $39 per ad account** — one free on annual plans — and it delivers exactly the
parts a project needs:

| This page's part | What the playbook gives you |
|---|---|
| **Instructions** | `AGENT_INSTRUCTIONS.md` — paste straight into the instructions field |
| **Context** | `references/account-profile.md` — a written dossier of the account |
| Prompt library | `PROMPTBOOK.md` — starting prompts against your real campaigns |
| Skill | `SKILL.md` — the operating doctrine, scoped to that one account |

It follows the golden rule above without you thinking about it: instructions are rules, the dossier
is data, and they arrive in separate files. Folder-based clients get an `AGENTS.md` they read
automatically. See [how to install it](/agent-skills/prompt-engineering-playbook#install-it-in-your-ai-client)
for each client.
...
<CardGroup cols={2}>
  ...
  <Card title="Prompt Engineering Playbook" icon="wand-magic-sparkles" href="/agent-skills/prompt-engineering-playbook">
    Instructions, dossier, and prompt book generated from one ad account. $39 one-time.
  </Card>
</CardGroup>
