---
title: "How Adspirer Implements MCP (Model Context Protocol)"
sidebarTitle: "How MCP Works"
description: "How Adspirer implements the Model Context Protocol: architecture, OAuth 2.1 with PKCE, streamable HTTP transport, tool discovery, and security."
icon: "diagram-project"
keywords: ["mcp", "model context protocol", "oauth", "architecture", "authentication"]
---
...
The server returns all 340+ tools with names, descriptions, and input schemas. The AI client uses these schemas to understand what parameters each tool accepts and to validate inputs before calling.
...
<Tree>
  <TreeItem title="mcp.adspirer.com" icon="server">
    ...
    <TreeItem title="JSON-RPC 2.0 Router" icon="route">
      <TreeItem title="tools/list — Discover 340+ tools" />
      <TreeItem title="tools/call — Execute tool with auth" />
    </TreeItem>
    <TreeItem title="Platform Adapters" icon="plug">
      <TreeItem title="Google Ads API (95 tools)" />
      <TreeItem title="Meta Marketing API (37 tools)" />
      <TreeItem title="Amazon Ads API (61 tools)" />
      <TreeItem title="ChatGPT Ads / OpenAI Ads API (31 tools)" />
      <TreeItem title="LinkedIn Marketing API (45 tools)" />
...
Two separate things. The **product MCP server** at `mcp.adspirer.com/mcp` connects AI assistants to ad platforms (340+ tools). The **docs MCP** at `www.adspirer.com/docs/mcp` is auto-generated by Mintlify and lets AI assistants search Adspirer's documentation. They serve different purposes.
</Accordion>
