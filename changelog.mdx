...
All performance and analysis tools across Google Ads, Meta Ads, LinkedIn Ads, and TikTok Ads now support a `raw_data` parameter. Pass `raw_data: true` to get clean JSON metrics without recommendations, severity labels, or commentary — useful when you want to process the numbers yourself or feed them into spreadsheets.

### How to use it

Ask your AI client for "raw data", "raw metrics", "just the numbers", or "no recommendations" — or pass `raw_data: true` directly in tool arguments.
...
Default is `false` (full formatted analysis with recommendations).
