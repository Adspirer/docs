...
<Update label="Meta Ads Standard Access disclaimer" date="2026-04-09" tags={["Platform", "Meta Ads", "Onboarding"]}>
Adspirer now displays a disclaimer on the connections page and onboarding flow confirming that the Adspirer App has Meta approved **Ads Management Standard Access**. The notice also clarifies that Meta's account enforcement systems (security reviews, restrictions, account disabling) operate independently from Adspirer and are governed by Meta's own Community Standards and policies.
</Update>
