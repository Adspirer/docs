...
<Steps>
  <Step title="Add the Adspirer Marketplace">
    ...
    ```
    This registers the Adspirer marketplace in Codex. No git or curl required.
  </Step>
  <Step title="Install the Plugin">
    ...
    Then type `/plugins` → navigate to the **Adspirer** tab → select **Adspirer Ads Agent** → press **Enter to install**.
  </Step>
  <Step title="Authenticate with Adspirer">
    On first use, a browser window opens automatically for OAuth. Sign in with your Adspirer account and authorize access. Return to your terminal when complete.
  </Step>
