...
| Authentication failed | **Log out of [adspirer.ai](https://adspirer.ai) first** — being logged in blocks the OAuth flow for CLI clients. Then run `codex mcp login adspirer` to re-authenticate |
| Install appears to hang | **Log out of [adspirer.ai](https://adspirer.ai) first.** A browser window may have opened for OAuth — complete sign-in and return to terminal |
