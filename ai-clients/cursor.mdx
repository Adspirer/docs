...
<Steps>
  <Step title="Verify the MCP Server is Connected">
    ...
    If `adspirer` is missing entirely, add it manually: **Cursor Settings > MCP > Add Server** with URL:

    ```
    https://mcp.adspirer.com/mcp
    ```
  </Step>
  <Step title="Open Your Brand Folder in Cursor">
    ```bash
  ...
  <Step title="Switch to Agent Mode and Set Up Your Workspace">
    Make sure you're in **Agent mode** (not Ask or Edit mode) in the Cursor chat panel. Then say:
