...
| Authentication failed | **Log out of [adspirer.ai](https://adspirer.ai) first** — being logged in blocks the OAuth flow for CLI/IDE clients. Then remove and re-add the MCP server in Cursor Settings |
| OAuth window doesn't open | **Log out of [adspirer.ai](https://adspirer.ai) first.** Also check browser pop-up blocker. Try restarting Cursor. |
