...
| Feature | Manus | Claude | ChatGPT | Cursor |
|---------|-------|--------|---------|--------|
| Auth method | API Key | OAuth | OAuth | OAuth |
| Autonomous tasks | Yes | Limited | Limited | No |
| Web research | Yes | No | Yes (Browse) | No |
| Dashboard generation | Yes | No | No | No |
| Scheduled briefs | Yes | No | No | No |
| Skill support | Skills (GitHub import) | `.claude/` | Custom GPT | `.cursor/` |
| Campaign management | Yes | Yes | Yes | Yes |
| Performance queries | Yes | Yes | Yes | Yes |
