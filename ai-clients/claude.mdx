...
For developer/terminal setup, see the full [Claude Code Setup Guide](/ai-clients/claude-code). The recommended path is the plugin install (bundles the MCP connector with slash commands, skills, and an agent):

```bash
/plugin install adspirer-ads-agent@claude-community
```

Prefer raw MCP access only? Use the manual fallback:
