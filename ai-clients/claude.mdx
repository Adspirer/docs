...
## Add the Adspirer Skill (Optional)

Claude now supports **Skills** — instruction files that teach Claude proven workflows for using your tools. Adding the Adspirer skill means Claude will automatically research keywords before creating campaigns, validate assets before launching, and follow platform-specific best practices.

<Steps>
  <Step title="Open Claude Skills">
    Go to [Customize](https://claude.ai/customize/connectors) in Claude, then click the **Skills** tab.
  </Step>
  <Step title="Upload the Adspirer Skill">
    Click **Upload skill** and paste the content from the [Adspirer SKILL.md file](https://github.com/amekala/ads-mcp/blob/main/skills/ad-campaign-management/SKILL.md).

    Or create a new skill with these details:
  ...
  <Step title="Verify">
    Start a new chat and ask Claude to create a campaign. With the skill installed, Claude will automatically follow the correct workflow — check connections, research keywords, validate assets, then create the campaign (paused) — instead of guessing the tool order.
  </Step>
</Steps>

<Tip>
Without the skill, Claude has access to 100+ tools but guesses the order. With the skill, Claude follows proven workflows with safety rules (campaigns always created paused, user confirmation before spend). See [Agent Skills Overview](/agent-skills/overview) for details.
</Tip>
