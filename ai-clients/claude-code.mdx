...
<Note>
**Plugin discovery has been removed from the terminal CLI in some recent Claude Code builds.** If `/plugin` doesn't surface Adspirer in your shell, use the **Manual MCP Setup** tab — same 400+ tools, just without the slash commands. The plugin path still works on Claude Code desktop / IDE-side.
