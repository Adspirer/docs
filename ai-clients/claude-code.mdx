...
Two ways to connect. **Plugin Install is the recommended path** — one command bundles the MCP connector with slash commands, skills, and a Performance Marketing Agent. Manual MCP Setup is available as a fallback if you only need raw tool access without the slash commands and agent.
...
<Tab title="Manual MCP Setup (fallback)">
<Steps>
  ...
  <Step title="Add Agent Skills (Optional)">
    This path only adds the MCP connector — no slash commands or agent. To get the skill file that gives Claude proven advertising workflows (campaign creation, performance review, wasted-spend audit), copy it manually:
