...
description: "Connect Claude Code to Adspirer MCP in 2 minutes. Manage Google Ads, Meta Ads, Amazon Ads, ChatGPT Ads, LinkedIn Ads, and TikTok Ads from your terminal with 300+ tools."
icon: "/icons/claude-code.svg"
keywords: ["claude code", "terminal", "cli", "mcp", "plugin"]
---

Manage all your ad campaigns from the terminal. One command to connect, 300+ tools to use.
...
Two ways to connect. **Manual MCP Setup is the recommended path** — one command, works on every Claude Code install. The Plugin Install path bundles the connector with slash commands, skills, and a Performance Marketing Agent, but plugin discovery has been removed from the terminal CLI in recent Claude Code builds (still available on Claude Code desktop / IDE-side). If `/plugin` doesn't show Adspirer, use Manual MCP Setup.
...
<Steps>
  <Step title="Add the MCP Server">
    ...
    Next time you ask Claude Code about your ad accounts, it'll prompt you to sign in. Your browser opens automatically for OAuth 2.1 authorization with PKCE.
  </Step>
  <Step title="Sign In to Adspirer">
    Approve the requested scopes (campaigns:read/write, reporting:read, accounts:read) and link your Google Ads, Meta Ads, LinkedIn Ads, and/or TikTok accounts. Return to your terminal — you'll see "Authentication successful".
  </Step>
  <Step title="Add Agent Skills (Optional)">
    The skill files give Claude proven advertising workflows (campaign creation, performance review, wasted-spend audit).
...
<Note>
**Plugin discovery has been removed from the terminal CLI in some recent Claude Code builds.** If `/plugin` doesn't surface Adspirer in your shell, use the **Manual MCP Setup** tab — same 300+ tools, just without the slash commands. The plugin path still works on Claude Code desktop / IDE-side.
...
<Check>
You're connected. Claude Code now has access to 300+ advertising tools from your terminal.
</Check>
...
300+ tools across 6 ad platforms. Here's what matters:
