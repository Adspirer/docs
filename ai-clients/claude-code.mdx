...
<Tip>
**Adspirer is now an official Claude plugin.** Search "adspirer" in the Claude Code plugin marketplace — no custom marketplace setup required.
</Tip>
...
<Steps>
  <Step title="Search and Install the Plugin">
    Type `/plugin` to open the plugin browser. In the **Discover** tab, search for `adspirer`. You'll see **adspirer-ads-agent** from `claude-plugins-official`.

    Select it, then choose **Install for you (user scope)** when prompted.
  ...
  <Step title="Authenticate with Adspirer">
    ...
    2. Find **plugin:adspirer-ads-agent:adspirer MCP Server** — it will show "needs authentication"
    3. Select it and press Enter — you'll see the server details with **Authenticate** option
    4. Select **Authenticate** — your browser opens for OAuth sign-in
    5. Sign in with your Adspirer account and authorize access
    6. Return to Claude Code — you'll see "Authentication successful"
  </Step>
...
Run `/reload-plugins` after installing, or restart Claude Code. Then type `/mcp` — you should see **plugin:adspirer-ads-agent:adspirer MCP Server** in the list.
