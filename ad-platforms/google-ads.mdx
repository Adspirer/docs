...
description: "Manage Google Search and Performance Max campaigns with AI. 40+ tools for keyword research, ad copy generation, performance analysis, and budget optimization."
...
The most comprehensive ad platform integration. Create Search and Performance Max campaigns, research keywords with real CPC data, generate ad copy, and optimize performance with 40+ specialized tools.
...
All performance tools (`get_campaign_performance`, `analyze_wasted_spend`, `optimize_budget_allocation`, `analyze_search_terms`, `explain_performance_anomaly`) accept an optional `raw_data` parameter. When set to `true`, the tool returns a compact JSON code block with only raw metrics (spend, clicks, impressions, conversions, CPA, CPC, CTR, CVR, ROAS) — no severity labels, suggested bids/budgets, industry benchmarks, or optimization recommendations.
...
```
Show me raw performance data for my Google Ads campaigns over the last 30 days — just the numbers, no recommendations.
```
