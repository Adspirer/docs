...
description: "Manage Facebook and Instagram campaigns with Adspirer's 36 MCP tools. Create image, video, carousel, and DCO ads, detect creative fatigue, optimize audiences, and analyze performance across Meta placements."
icon: "image"
keywords: ["meta ads", "facebook ads", "instagram ads", "social media ads", "carousel", "dco"]
---
...
- `analyze_meta_audiences` — See which audience segments perform best by demographics
- `get_meta_audience_insights` — Audience demographics, placement breakdown, and device targeting insights
- `analyze_meta_wasted_spend` — Identify underperforming campaigns, placement waste, and creative fatigue
...
- `create_meta_dco_ad` — Create a Dynamic Creative Optimization ad that lets Meta automatically test multiple images to find the best combination

### Ad Set & Ad Management

- `add_meta_ad_set` — Add a new ad set to an existing campaign (supports image, video, carousel, DCO, CBO spend controls, and custom conversion tracking)
- `add_meta_ad` — Add another ad/creative variation to an existing ad set
- `list_meta_ad_sets` — View ad sets within a campaign with targeting, budgets, and optimization settings
- `list_meta_ads` — View individual ads within an ad set with status and creative info
- `update_meta_ad_set` — Edit targeting, budget, bid, placements, schedule, or optimization settings
- `update_meta_ad` — Update an individual ad (pause/resume, rename, or swap creative)

### Campaign Management

- `list_meta_campaigns` — Browse all Meta campaigns with status and budget info
- `get_meta_campaign_details` — View full campaign structure including ad sets, ads, targeting, and budgets
...
### Account & Creative Tools

- `list_meta_instagram_accounts` — Find Instagram account IDs for running Instagram ads
- `list_meta_pixels` — View Meta Pixels for conversion tracking setup
- `get_meta_ad_creatives` — See ad creatives, ad copy, and media URLs
- `list_meta_lead_forms` — Browse lead generation forms
- `get_meta_lead_form_submissions` — View lead data collected from Meta lead forms
...
<AccordionGroup>
  ...
  <Accordion title="How much does Adspirer cost for Meta Ads?">
    Adspirer pricing is based on tool calls, not ad spend. Free tier: 15 calls/month. Plus: $49/mo for 150 calls. Pro: $99/mo for 600 calls. Max: $199/mo for 3,000 calls. A typical Meta campaign creation uses 4-6 tool calls. See [full pricing](https://www.adspirer.com/pricing?utm_source=docs&utm_medium=page&utm_content=pricing).
  </Accordion>
</AccordionGroup>
