...
description: "Manage Facebook and Instagram campaigns with Adspirer's 23 MCP tools. Create image, video, and carousel ads, add ad sets and ads to existing campaigns, detect creative fatigue, optimize audiences, and analyze performance across Meta placements."
icon: "image"
...
- `add_meta_ad_set` — Add a new ad set with targeting and creative to an existing campaign (supports lifetime budgets, end dates, placement controls, multi-advertiser opt-out)
- `add_meta_ad` — Add a new ad creative to an existing ad set (supports multi-advertiser opt-out, placement-specific creatives)
...
Once a campaign is live, use `add_meta_ad_set` to add new ad sets with different targeting or budgets to the same campaign, and `add_meta_ad` to add new creatives to an existing ad set.

- **Different audiences or budgets** → same campaign, different ad sets → use `add_meta_ad_set`
- **Different creatives, same audience** → same ad set, different ads → use `add_meta_ad`

`add_meta_ad_set` supports all the same options as campaign creation: lifetime budgets, end dates, placement controls (`publisher_platforms`, `facebook_positions`, `instagram_positions`), and multi-advertiser opt-out.
