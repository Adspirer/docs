...
<AccordionGroup>
  <Accordion title="Nothing is working — tools fail or I can't reach Adspirer">
    ...
    **Step 2: Disconnect and reconnect the Adspirer connector**
    ...
    - **Claude Code (plugin install, recommended):** Type `/plugin` → **Installed** tab → select **adspirer** → **Authenticate** (or use `/mcp` → **plugin:adspirer-ads-agent:adspirer MCP Server** → **Authenticate**)
    - **Claude Code (manual MCP install):** Run `claude mcp remove adspirer`, then `claude mcp add --transport http adspirer https://mcp.adspirer.com/mcp`, restart, and re-authenticate via `/mcp`
