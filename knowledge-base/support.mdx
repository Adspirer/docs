...
<Steps>
  ...
  <Step title="Disconnect and Reconnect the Connector">
    ...
    - **Claude Code (plugin install, recommended):** `/plugin` → **Installed** tab → **adspirer** → **Authenticate** (or `/mcp` → **plugin:adspirer-ads-agent:adspirer MCP Server** → **Authenticate**)
    - **Claude Code (manual MCP install):** `claude mcp remove adspirer` → `claude mcp add --transport http adspirer https://mcp.adspirer.com/mcp` → Restart → `/mcp` to authenticate
