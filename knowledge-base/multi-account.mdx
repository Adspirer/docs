...
description: "Manage multiple ad accounts across Google, Meta, LinkedIn, and TikTok Ads from one Adspirer connection — Max activates up to 5 accounts at once."
icon: "building"
keywords: ["multi-account", "agency", "multiple accounts", "account management", "sequential sub-agents", "max plan"]
---

You can connect **unlimited ad accounts** across all plans — there is no restriction on how many accounts you link. The difference between plans is how many accounts can be **active** (used by the agent) at once:

- **Free, Plus, Pro** — one active account per platform at a time. Switch between them using `switch_primary_account`.
- **Max** — up to 5 accounts active simultaneously. Sequential sub-agents fan out tool calls across all active accounts in a single conversation.
...
Teams that need more than 5 active accounts — or multi-seat access pooled across planners — should look at the [Adspirer Enterprise plans for agencies and teams](/knowledge-base/enterprise), which scale sequential sub-agents across up to 200 ad accounts.
...
No — you can connect unlimited ad accounts on **any** plan. The difference is how many can be **active** at once. Free, Plus, and Pro keep one active account per platform at a time — switch with `switch_primary_account`. The Max plan activates up to 5 accounts simultaneously, with sequential sub-agents fanning out tool calls across all of them in a single prompt. Use Max when you regularly run cross-account audits, briefs, or bulk optimizations.
</Accordion>
