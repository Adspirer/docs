...
On **Max**, you're an individual marketer with unlimited tool calls, up to 5 ad accounts per platform, and sequential subagents that can automate across all 5.
...
Connect up to 50 ad accounts per platform (Enterprise) or 200 (Enterprise Plus). Each account's data is synced and persisted — the agent has full context on every client.
</Card>
...
A single enterprise ad account can contain thousands of campaigns, millions of keywords, and years of performance history. The infrastructure cost scales with accounts, not just tool calls. That's why Max includes 5 accounts per platform, and Enterprise tiers scale up to 50–200.

<Tip>
**Max (5 accounts/platform)** is ideal for a solo marketer managing one business across multiple platforms. **Enterprise** is for teams managing multiple clients — each planner works on their assigned accounts, with sequential subagents automating across all of them.
</Tip>
...
| Tier | Accounts per Platform | Sequential Subagent Scope |
|------|----------------------|--------------------------|
...
**Max** plans sync and persist the **last 90 days** of campaign data per connected ad account. **Enterprise** and **Enterprise Plus** plans extend this to **12 months** — giving the agent full context for trend analysis, seasonal comparisons, and year-over-year reporting.
