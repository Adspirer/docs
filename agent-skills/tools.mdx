...
description: "Complete reference of all 130+ Adspirer tools. Organized by platform (Google Ads, Meta Ads, LinkedIn Ads, TikTok Ads) with types, descriptions, and parameters."
icon: "wrench"
mode: "wide"
...
All performance and analytics tools across every platform accept an optional `raw_data` parameter (default: `false`). When set to `true`, the tool returns a compact JSON code block containing only raw metrics — severity labels, suggested bids/budgets, industry benchmarks, and optimization recommendations are stripped.

Use raw data mode when you run your own attribution model, want to pipe data into a spreadsheet, or want to minimize token usage. Just ask for "raw data", "raw metrics", "just the numbers", or "no recommendations" in your prompt.

Supported on 30 tools across Google Ads (5), Meta Ads (10), LinkedIn Ads (7), and TikTok Ads (8).
