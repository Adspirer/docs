...
All performance and analysis tools across every platform support `raw_data: true`. When enabled, the tool returns a JSON code block with metrics only (spend, clicks, impressions, conversions, CPA, CPC, CTR, CVR, ROAS) — severity labels, suggested bids, industry benchmarks, and optimization recommendations are stripped. Default is `false`. Ask for "raw data", "raw metrics", "just the numbers", or "no recommendations" and your AI client passes the flag automatically.
