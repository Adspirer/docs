...
| Tool Name | Type | Description |
|-----------|------|-------------|
| `list_connected_accounts` | <Badge color="green">Read</Badge> | List all connected ad accounts across platforms with IDs, names, and tiers. Call this at the start of every session to discover available accounts. Accepts optional `platform` filter. Zero API calls — reads from database. |
...
<Note>
- **Always start with `list_connected_accounts`:** Discover available accounts and their IDs before calling any platform tool. For multi-account users (agencies), this is critical — pass the returned account ID to every subsequent tool call.
