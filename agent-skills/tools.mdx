...
| `list_connected_accounts` | <Badge color="green">Read</Badge> | List all active ad accounts across platforms — used by Max-tier sequential sub-agents to fan out work across accounts. Accepts optional `platform` filter. Zero API calls (reads from database). |
